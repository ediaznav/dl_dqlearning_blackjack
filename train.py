...
# Librerias de Environment y auxiliares
...
# Configuración e hiperparametros
...
def train(config: HyperParams):
    ...
    # Importamos la clase QNetwork para mi agente 
    if config.DUELING:
    ...
    for episode in range(N_EPISODES):
        ...
        while not done:
            ...
            log_step(episode, step_id, 
                    state, action, reward, agent.epsilon, env.bankroll, base_dir=DIRECTORY_LOGS)
        ...
        # Reset bankroll if needed
        if env.bankroll <= 0:
            env.bankroll = env.initial_bankroll

        reward_history.append(episode_reward)

        # TensorBoard per-episode summary
