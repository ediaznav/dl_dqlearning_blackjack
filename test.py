...
# Este script continene el codigo necesario para ejecutar una prueba o test
# dada un modelo seleccionado . 
# 
...
# Funciones para logging (no usadas aquí pero importadas)
...
# from blackjack_ui import BlackjackUI  # Interfaz opcional si se activa render

### CONFIGURACIÓN DE EVALUACIÓN --------------------------------------------

# Ruta al modelo entrenado (ajustar si es necesario)
MODEL_PATH = "models/q_network_dueling_b845945d4b.pth"
...
### MÉTRICAS Y VARIABLES DE EVALUACIÓN --------------------------------------

# Diccionarios para contar resultados y registrar eventos
...
def interpret_result(player_val, dealer_val, reward):
    """
    Devuelve el resultado textual de un episodio con base en los valores
    de las manos del jugador, del dealer y la recompensa final.
...
for episode in range(N_EPISODES):
    ...
    # Evaluar resultado del episodio
...
# Guardar resumen en CSVs
pd.DataFrame(results, index=[list(results.keys())]).to_csv(
    RESUlTS_PATH + "results_duel.csv", index=False)

pd.DataFrame(eventos).to_csv(
    RESUlTS_PATH + "eventos_duel.csv", index=False)

pd.DataFrame({
    "dealer_hand": states_d,
    "player_hand": states_p,
    "action": actions
